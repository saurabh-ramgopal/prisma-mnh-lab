...
function category_tag_dropdown_filter_and_posts() {
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            $query->the_post(); // Properly set up post data
            echo '<div class="pr-card__work-col">';
            echo '<div class="post-card pr-card__story">';
            echo '<div class="pr-d-md-flex">';
            echo '<div class="post-card-left pr-shrink-0 line-height-0">';
            echo '<a target="_blank" class="pr-card__story-img-wrap" href="' . get_the_permalink() . '">';
            if (has_post_thumbnail()) {
                echo '<img src="' . get_the_post_thumbnail_url() . '" />';
            } else {
                echo '<img src="/wp-content/uploads/2024/06/placeholder-headshot.png" />';
            }
            echo '</a>';
            echo '</div>';
            echo '<div class="post-card-right">';
            echo '<h2 class="pr-card__title pr-card__title-bold pr-mb-3"><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h2>';
            echo '<p class="pr-card__para">' . wp_trim_words(get_the_excerpt(), 60) . ' <a target="_blank" href="' . get_the_permalink() . '" >show more</a></p>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
...
//     }
    ...
    if($query->have_posts()) {
        while($query->have_posts()) {
            ...
            echo '</div>';
			echo '<div class="post-card-right">';
            echo '<h2 class="pr-card__title pr-card__title-bold pr-mb-3"><a class="pr-d-flex" target="_blank" href="' . get_post_meta(get_the_ID(), 'external_link', true) . '">' . get_the_title() . '<span class="pr-shrink-0" style="margin-left: 12px;"><img src="/wp-content/uploads/2024/06/maximize-icon.png" /></span></a></h2>';
            echo '<p class="pr-card__para">' . wp_trim_words( get_the_excerpt(), 10, '...' ) . '</p>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
        }
...
// 			// Add the post thumbnail section
// 			echo '<div class="post-card-left pr-shrink-0 line-height-0">';
// 			echo '<a target="_blank" class="pr-card__story-img-wrap" href="' . get_post_meta(get_the_ID(), 'external_link', true) . '">';
// 			if (has_post_thumbnail()) {
// 				echo '<img src="' . get_the_post_thumbnail_url() . '" />';
// 			} else {
// 				echo '<img src="/wp-content/uploads/2024/06/placeholder-headshot.png" />';
...
// 			// Add the post title and excerpt section
// 			echo '<div class="post-card-right">';
// 			echo '<h2 class="pr-card__title pr-card__title-bold"><a class="pr-d-flex" target="_blank" href="' . get_post_meta(get_the_ID(), 'external_link', true) . '">' . get_the_title() . '<span class="pr-shrink-0" style="margin-left: 12px;"><img src="/wp-content/uploads/2024/06/maximize-icon.png" /></span></a></h2>';
// 			echo '<p class="pr-card__para">' . wp_trim_words(get_the_excerpt(), 42, '...') . '</p>';
// 			echo '</div>';

// 			echo '</div>'; // Close the flex container
// 			echo '</div>'; // Close the post card
// 		}
// 	} else {
// 		echo 'No posts found';
// 	}
...
function add_partner_external_link_meta_box() {
    add_meta_box(
        'partner_external_link_meta_box', // ID
        'External Link',                  // Title
        'partner_external_link_callback', // Callback function
        'partners-and-funder',            // Post type
        'normal',                         // Context
        'high'                            // Priority
    );
}

function partner_external_link_callback($post) {
    // Fetch existing external link value
    $external_link = get_post_meta($post->ID, '_partner_external_link', true);
    ?>
    <label for="partner_external_link"><strong>Add External Link:</strong></label>
    <input type="url" 
           id="partner_external_link" 
...
function save_partner_external_link_meta_box($post_id) {
    // Save external link if provided
    if (isset($_POST['partner_external_link'])) {
        update_post_meta($post_id, '_partner_external_link', esc_url_raw($_POST['partner_external_link']));
    }
}

// Hook into WordPress
add_action('add_meta_boxes', 'add_partner_external_link_meta_box');
add_action('save_post', 'save_partner_external_link_meta_box');
...
function partners_and_funders_shortcode($atts) {
    ...
    while ($query->have_posts()) {
        ...
        $external_link = get_post_meta(get_the_ID(), '_partner_external_link', true); // Fetch external link
        ?>
        <div class="grid-item-pf pr-card__wrap">
            <a href="<?php echo esc_url($external_link ? $external_link : '#'); ?>" target="_blank" class="pr-card__title-wrap pr-d-flex">
                <div class="pr-card__thumb pr-shrink-0">
                    <img src="<?php echo esc_url($image_url); ?>" alt="<?php the_title(); ?>" />
                </div>
                <div class="pr-grow-1 pr-card__thumb-content">
                    <div class="pr-d-flex pr-items-start pr-content-between">
                        <h2 class="pr-card__title pr-card__title-partners"><span><?php the_title(); ?></span></h2>
                        <p class="pr-shrink-0 pr-cate pr-cate--<?php echo esc_attr(str_replace(',', '', $term_class_name)); ?>">
