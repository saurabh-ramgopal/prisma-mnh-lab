...
function otw_sw_add_shortcode( controler ){
	...
	if( id_matches = controler.id.match( "^(.*)otw-sw-add-shortcode$" ) ){
		...
		var wp_url = otw_post_template_shortcode_component.wp_url;
		...
		if( code_object.shortcodes.length == 0 ){
			...
			jQuery.post( 'admin-ajax.php?action=otw_post_template_shortcode_editor_dialog&shortcode=' + shortcode_type, { shortcode_object: post_data }, function(b){
								...
								otw_shortcode_editor = new otw_post_template_shortcode_editor_object( shortcode_type );
								...
								tb_show( ' OTW ' + otw_post_template_shortcode_component.shortcodes[ shortcode_type ].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
							} );
...
function otw_sw_get_label( string ){
	...
	return otw_post_template_shortcode_component.get_label( string );
}