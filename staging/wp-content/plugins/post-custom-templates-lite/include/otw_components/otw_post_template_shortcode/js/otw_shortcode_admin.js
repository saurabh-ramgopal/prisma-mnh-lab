...
function otw_post_template_shortcode_object(){
...
otw_post_template_shortcode_object.prototype.open_drowpdown_menu = function( append_to ){
	
	this.dropdown_menu = jQuery( '#otw_post_template_shortcode_dropdown_menu' );
	
	if( !this.dropdown_menu.size() ){
	
		var links = '<div id=\"otw_post_template_shortcode_dropdown_menu\">';
...
otw_post_template_shortcode_object.prototype.insert_code = function( shortcode_object ){
	
};

otw_post_template_shortcode_object.prototype.load_shortcode_editor_dialog = function( otw_post_template_shortcode_name, options = true ){

	var oSelf = this;
	
	if( options == false ){
		
		jQuery.post( oSelf.wp_url + '.php?action=otw_post_template_shortcode_get_code&shortcode=' + otw_post_template_shortcode_name,{'shortcode_type': otw_post_template_shortcode_name},function( response ){
			
			var response_code = jQuery.parseJSON( response );
			
			var code = {};
			code.shortcode_code = '';
			code.shortcode_type = otw_post_template_shortcode_name;
			...
			if( !response_code.has_error ){
				...
				oSelf.insert_code( code );
			}else{
				oSelf.shortcode_error( response_code.errors );
			};
		});
	}else{
		jQuery.get( oSelf.wp_url + '.php?action=otw_post_template_shortcode_editor_dialog&shortcode=' + otw_post_template_shortcode_name,function(b){
			...
			otw_post_template_shortcode_editor = new otw_post_template_shortcode_editor_object( otw_post_template_shortcode_name );
			otw_post_template_shortcode_editor.preview = oSelf.shortcodes[ otw_post_template_shortcode_name ].object.preview;
			...
			tb_show( oSelf.get_label( 'Insert' ) + ' OTW ' + oSelf.shortcodes[ otw_post_template_shortcode_name ].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
			
		} );
	}
};

otw_post_template_shortcode_object.prototype.init_dropdown_actions = function(){
	...
	jQuery( 'body' ).on( 'click',  function( event ){
		...
		if( jQuery( event.target ).parents( '#otw_post_template_shortcode_dropdown_menu' ).size() ){
			close_it = false;
		};
				...
				switch( matches[1] ){
					...
					default:
							if( jQuery( this ).attr( 'data-options' ) == 'no' ){
								...
								oSelf.load_shortcode_editor_dialog( matches[1], false );
								oSelf.dropdown_menu.hide();
							}else{
								oSelf.load_shortcode_editor_dialog( matches[1], true );
								oSelf.dropdown_menu.hide();
							}
						break;
				};
...
otw_post_template_shortcode_object.prototype.get_label = function( label ){
...
function otw_post_template_shortcode_editor_object( type ){
					...
					if( editor && editor instanceof tinymce.Editor ) {
						...
						if( jQuery( '#otw-shortcode-element-' + id_matches[1] ).attr( 'data-loaded' ) != 1 ){
							...
							editor.onChange.add( function(){
								...
								otw_post_template_shortcode_editor.live_preview();
							} );
						}
...
otw_post_template_shortcode_editor_object.prototype.init_html_areas = function(){
...
otw_post_template_shortcode_editor_object.prototype.init_action_buttons = function(){
...
otw_post_template_shortcode_editor_object.prototype.live_reload = function(){
	...
	jQuery.post( this.wp_url + '.php?action=otw_post_template_shortcode_live_reload&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
		...
		jQuery( '#TB_ajaxContent' ).find( '#otw-shortcode-btn-insert' ).val( soSelf.ave_button_value );
		jQuery( '#TB_ajaxContent' ).find( '#otw-shortcode-btn-insert-bottom' ).val( oSelf.save_button_value );
	});
};
otw_post_template_shortcode_editor_object.prototype.live_preview = function(){
	...
	jQuery.post( this.wp_url + '.php?action=otw_post_template_shortcode_live_preview&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': this.post_id }, function( response ){
...
otw_post_template_shortcode_editor_object.prototype.shortcode_error = function( errors ){
...
otw_post_template_shortcode_editor_object.prototype.get_values = function(){

	var v_code = {};
	v_code.shortcode_code = '';
	v_code.shortcode_type = this.shortcode_type;
	
	for( var field in otw_post_template_shortcode_editor.fields ){
	
		var matches = false;
		if( matches = field.match( /^otw\-shortcode\-element\-([a-z0-9\_\-]+)$/ ) ){
			
			switch( otw_post_template_shortcode_editor.fields[ field ].element_type ){
				
				case 'checkbox':
						
						if( otw_post_template_shortcode_editor.fields[ field ].element[0].checked == true ){
							v_code[ matches[1] ] = otw_post_template_shortcode_editor.fields[ field ].current_value;
						}else{
							v_code[ matches[1] ] = '';
						}
					break;
				case 'text_area':
						...
						if( ( otw_post_template_shortcode_editor.fields[ field ].element.attr( 'data-type' ) == 'tmce' ) && ( otw_post_template_shortcode_editor.fields[ field ].element.attr( 'data-loaded' ) == 1 ) ){
							...
							if( tinyMCE.get( 'otw-shortcode-element-' + matches[1] + '_tmce' ) != null ){
								...
								otw_post_template_shortcode_editor.fields[ field ].element.val( v_code[ matches[1] ] );
			...
			v_code.otw_item_id = otw_post_template_shortcode_editor.fields[ field ].current_value;
		}
	};
...
otw_post_template_shortcode_editor_object.prototype.get_code = function(){
	...
	jQuery.post( this.wp_url + '.php?action=otw_post_template_shortcode_get_code&shortcode=' + this.shortcode_type , this.code, function( response ){
...
otw_post_template_shortcode_editor_object.prototype.init_fields = function(){
	...
	jQuery( '.otw-shortcode-editor-fields' ).find( 'input[type=text]' ).each( function(){
		...
		if( element.attr( 'id' ) ){
			oSelf.fields[ element.attr( 'id' ) ] = new otw_post_template_shortcode_editor_element( 'text_input', element );
		}
	...
	jQuery( '.otw-shortcode-editor-fields' ).find( 'input[type=hidden]' ).each( function(){
		...
		if( element.attr( 'id' ) ){
			oSelf.fields[ element.attr( 'id' ) ] = new otw_post_template_shortcode_editor_element( 'hidden_input', element );
		}
	} );
	jQuery( '.otw-shortcode-editor-fields' ).find( 'input[type=checkbox]' ).each( function(){
		...
		if( element.attr( 'id' ) ){
			oSelf.fields[ element.attr( 'id' ) ] = new otw_post_template_shortcode_editor_element( 'checkbox', element );
		}
		element.on( 'change',  function(){
			oSelf.live_preview();
		});
	...
	jQuery( '.otw-shortcode-editor-fields' ).find( 'select' ).each( function(){
		...
		if( element.attr( 'id' ) ){
			oSelf.fields[ element.attr( 'id' ) ] = new otw_post_template_shortcode_editor_element( 'select', element );
		}
		element.on( 'change',  function(){
	...
	jQuery( '.otw-shortcode-editor-fields' ).find( 'textarea' ).each( function(){
		...
		if( element.attr( 'id' ) ){
			oSelf.fields[ element.attr( 'id' ) ] = new otw_post_template_shortcode_editor_element( 'text_area', element );
		}
		element.on( 'change',  function(){
			oSelf.live_preview();
		});
...
function otw_post_template_shortcode_editor_element( element_type, element ){
...
{
	var otw_post_template_shortcode_component = null;
}
otw_post_template_shortcode_component = null;
if( typeof( otw_post_template_shortcode_editor ) == undefined )
{
	var otw_post_template_shortcode_editor = null;
}
otw_post_template_shortcode_editor = null;
